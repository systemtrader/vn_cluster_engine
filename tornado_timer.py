...
from utilities.my_datetime import datetime2posix_timestamp,posix_timestamp2datetime

class tornado_timer(object):
	'''
	根据tornado的delayed_callback设计的定时器类
	'''
	
	def __init__(self, first_emit_moment=datetime.now(), emit_interval=None, emit_function=None,
	...
	def __set_single_call(self, time_to_call):
		...
		ioloop.IOLoop.current().call_at(posix_next_moment, self.__time_out)
		pass
